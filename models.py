...
def fit_model(model, patience, x_train, y_train, x_test, y_test):
    print("\nTraining model...\n")
    cb = EarlyStopping(monitor='val_loss', patience=patience)
    model.fit(x_train, y_train, epochs=1000, batch_size=32, callbacks=[cb], class_weight=_CLASS_WEIGHTS,
